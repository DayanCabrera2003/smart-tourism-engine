"""Crea la colección Qdrant `destinations_image` para embeddings CLIP (T082).

Esquema de la colección:
  - nombre:    destinations_image
  - dimensión: 512  (clip-ViT-B-32)
  - métrica:   Coseno
  - payload por punto:
      destination_id  str   — id del destino en destinations.db
      image_path      str   — ruta relativa a data/raw/images/
      name            str   — nombre del destino (para búsqueda rápida)
      country         str   — país del destino

Uso:
    python scripts/init_qdrant_images.py            # crea si no existe
    python scripts/init_qdrant_images.py --recreate # borra y vuelve a crear
    python scripts/init_qdrant_images.py --url http://localhost:6333
"""
...
VECTOR_SIZE = ClipEmbedder.DIMENSION
DISTANCE = "Cosine"


def init(store: VectorStore, *, recreate: bool = False) -> None:
    """Crea (o recrea) la colección de imágenes en el ``VectorStore`` dado."""
...
def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Inicializa la colección Qdrant de imágenes para CLIP."
    )
    parser.add_argument(
        "--recreate",
        action="store_true",
