...
| `region` | `Optional[str]` | Región, estado o provincia. |
| `description` | `str` | Descripción textual extensa del destino. |
| `description_normalized` | `Optional[str]` | Versión sin acentos, en minúsculas, usada por el matcher léxico para no depender de la forma original cuando el usuario escribe sin tildes. |
| `tags` | `List[str]` | Etiquetas o categorías (ej. playa, museos). |
| `image_urls` | `List[str]` | URLs absolutas o paths locales a imágenes representativas. Se acepta `str` (no `HttpUrl`) porque el script de descarga de imágenes guarda paths relativos a `data/raw/images/`. |
| `coordinates` | `Optional[Tuple[float, float]]` | Latitud y longitud geográfica. |
| `source` | `str` | Nombre de la fuente (`wikivoyage`, `wikidata+wikipedia`, `tavily`, ...). |
| `fetched_at` | `datetime` | Marca de tiempo de adquisición. |
| `popularity` | `Optional[float]` | Score en `[0, 1]` calculado por `scripts/compute_popularity.py` a partir de la longitud de descripción y las menciones cruzadas en el resto del corpus. El reranker de posicionamiento lo usa como factor secundario; el capítulo 13 detalla la fórmula. |

### Validación
El modelo utiliza validación estricta de tipos y asegura que los campos críticos no estén vacíos. Se pueden encontrar las pruebas correspondientes en `tests/test_ingestion.py`.
